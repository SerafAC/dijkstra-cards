...
import { FindOptimalSellers } from '../../bindings/dijkstra-cards/src/assignment/sellerassignmentservice'
import type { CardQuery } from '../../bindings/dijkstra-cards/src/cards-platform/models'
import { GetCardSellers, GetFetchStatuses } from '../../bindings/dijkstra-cards/src/cards-platform/cardmarket/cardmarketservice'
import { DataTable } from 'primevue'
import ProgressBar from 'primevue/progressbar'
import { Card, Seller } from '../../bindings/dijkstra-cards/src/models'
import { Browser } from '@wailsio/runtime'
import { SellerFetchStatus } from '../../bindings/dijkstra-cards/src/cards-platform/cardmarket'
...
const fetchPercentage = computed(() => hasCards.value ? 
    Math.round((fetchProgress.value / selectedCards.value.length) * 100) : 0)
...
async function assignSellers() {
  ...
  try {
    ...
    cardFetchErrors.value = (await GetFetchStatuses(selectedCards.value.map((card) => card.Id)))
...
<template>
  <div class="search-page">
    <div class="header">
      <h2>Search Cards</h2>
      <div class="actions">
        <Button label="Back to Deck" icon="pi pi-arrow-left" severity="secondary" outlined @click="goBack" />
        <Button label="Reset selection" icon="pi pi-refresh" severity="danger" outlined @click="resetSelection" />
      </div>
    </div>
    ...
    <div v-if="hasCards" class="results">
      ...
      <div v-if="hasFailedCards" class="failed-section">
        ...
        <p class="failed-section__summary">
          {{ failedCards.length === 1 ? 'This card could not fetch sellers.' : 'These cards could not fetch sellers.' }}
        </p>
      ...
      <div class="results-table">
        ...
        <div v-else class="results-placeholder">
          <p v-if="searchAttempted && !isSearching">No seller assignments available for the selected cards.</p>
